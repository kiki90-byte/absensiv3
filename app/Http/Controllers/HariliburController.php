...
class HariliburController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query->join('unit', 'hari_libur.kode_unit', '=', 'unit.kode_unit');
        if (!empty($request->kode_unit)) {
            $query->where('hari_libur.kode_unit', $request->kode_unit);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($user->hasRole(['super admin', 'admin pusat'])) {
            $validationRules['kode_unit'] = 'required';
        }
    ...
    public function aturharilibur($kode_libur)
    {
        ...
        $data['harilibur'] = Harilibur::where('kode_libur', $kode_libur)
            ->join('unit', 'hari_libur.kode_unit', '=', 'unit.kode_unit')
            ->first();
    ...
    function getkaryawan(Request $request)
    {
        ...
        $query->select('karyawan.nik', 'karyawan.nama_karyawan', 'harilibur.nik as ceklibur', 'nama_div');
        $query->join('divisi', 'karyawan.kode_div', '=', 'divisi.kode_div');
        if (!empty($request->kode_div)) {
            $query->where('karyawan.kode_divt', $request->kode_div);
        }
        $query->where('karyawan.kode_unit', $harilibur->kode_unit);
    ...
    public function tambahkansemua(Request $request)
    {
        ...
        $harilibur = Harilibur::where('kode_libur', $kode_libur)
            ...
            ->first();
        $query = Karyawan::query();
        $query->select('karyawan.nik', 'karyawan.nama_karyawan', 'karyawan.kode_div', 'nama_div', 'harilibur.nik as ceklibur');
    ...
    public function batalkansemua(Request $request)
    {
        ...
        $query->join('divisi', 'karyawan.kode_divisi', '=', 'divisi.kode_div');
        $query->orderBy('nama_karyawan');
        $karyawan = $query->get();
