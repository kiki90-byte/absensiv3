                        ...
                        <form action="{{ route('harilibur.index') }}" method="GET">
                            ...
                            @if ($user->hasRole(['super admin', 'gm administrasi']))
                                <div class="form-group">
                                    <select name="kode_unit_search" id="kode_unit_search" class="form-select select2Kodeunitsearch">
                                        <option value="">Semua Unit</option>
                                        @foreach ($unit as $c)
                                            <option value="{{ $c->kode_unit }}"
                                                {{ Request('kode_unit_search') == $c->kode_unit ? 'selected' : '' }}>
                                                {{ textUpperCase($c->nama_unit) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
...
<script>
    $(function() {
        const select2Kodeunitsearch = $(".select2Kodeunitsearch");
        if (select2Kodeunitsearch.length > 0) {
            select2Kodeunitsearch.each(function() {
                var $this = $(this);
