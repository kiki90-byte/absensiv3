...
class LaporanController extends Controller
{
    public function presensi()
    {
        ...
        $divisi = Divisi::orderBy('kode_div')->get();
        $data['unit'] = $unit;
        $data['divisi'] = $divisi;
    ...
    public function cetakpresensi(Request $request)
    {
        ...
        $q_presensi->leftJoin('divisi', 'karyawan.kode_div', '=', 'divisi.kode_div');
        ...
        if (!empty($request->kode_div)) {
            $q_presensi->where('karyawan.kode_div', $request->kode_div);
        }
        ...
        if (!empty($request->nik) && $request->format_laporan == 1) {
            ...
            $laporan_presensi = $presensi->groupBy('nik')->map(function ($rows) use ($jenis_tunjangan) {
                $data = [
                    ...
                    'kode_div' => $rows->first()->kode_div,
                    'nama_div' => $rows->first()->nama_div,
