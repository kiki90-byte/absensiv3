...
class PresensiController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        //Cek Lokasi Kantor
        $lokasi_kantor = Unit::where('kode_unit', $karyawan->kode_unit)->first();
        ...
        if ($kode_jam_kerja == null) {
            ...
            if ($jamkerja == null) {
                ...
                if ($jamkerja == null) {
                    $jamkerja = Detailsetjamkerjabydiv::join('presensi_jamkerja_bydiv', 'presensi_jamkerja_bydiv_detail.kode_jk_div', '=', 'presensi_jamkerja_bydiv.kode_jk_div')
                        ->join('presensi_jamkerja', 'presensi_jamkerja_bydiv_detail.kode_jam_kerja', '=', 'presensi_jamkerja.kode_jam_kerja')
    ...
    public function store(Request $request)
    {
        ...
        //Get Lokasi Kantor
        $unit = Unit::where('kode_unit', $karyawan->kode_unit)->first();
        // $lokasi_kantor = $unit->lokasi_unit;
        $lokasi_kantor = $request->lokasi_unit;
        ...
        //dd($presensi_hariini);
        if ($status_lock_location == 1 && $radius > $unit->radius_unit) {
    ...
    public function show($id, $status)
    {
        $presensi = Presensi::where('id', $id)
            ...
            ->first();
        $unit = Unit::where('kode_unit', $presensi->kode_unit)->first();
        $lokasi = explode(',', $unit->lokasi_unit);
        $data['latitude'] = $lokasi[0];
        $data['longitude'] = $lokasi[1];
        // if (!empty($presensi->lokasi_unit)) {
        //     $lokasi = explode(',', $presensi->lokasi_unit);
        //     $data['latitude'] = $lokasi[0];
        //     $data['longitude'] = $lokasi[1];
        // } else {
        //     $data['latitude'] = $unit->latitude_unit;
        //     $data['longitude'] = $unit->longitude_unit;
        // }
        $data['presensi'] = $presensi;
        $data['status'] = $status;
        $data['unit'] = $unit;
