...
class LemburController extends Controller
{
    public function index(Request $request)
    {
        ...
        $qlembur->join('divisi', 'karyawan.kode_div', '=', 'divisi.kode_div');
        $qlembur->join('unit', 'karyawan.kode_unit', '=', 'unit.kode_unit');
        ...
        if (!empty($request->kode_unit)) {
            $qlembur->where('karyawan.kode_unit', $request->kode_unit);
        }

        if (!empty($request->kode_div)) {
            $qlembur->where('karyawan.kode_div', $request->kode_div);
        }
    ...
    public function storepresensi(Request $request)
    {
        ...
        //Get Lokasi Kantor
        $lokasi_kantor = $lembur->lokasi_unit;
        ...
        if ($status_lock_location == 1 && $radius > $lembur->radius_unit) {
