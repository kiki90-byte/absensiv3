<form action="{{ route('karyawan.update', Crypt::encrypt($karyawan->nik)) }}" id="formcreateKaryawan" method="POST" enctype="multipart/form-data">
    ...
    <x-select-label label="Kantor Unit" name="kode_unit" :data="$unit" key="kode_unit" textShow="nama_unit"
        selected="{{ $karyawan->kode_unit }}" />
    <x-select-label label="Divisi" name="kode_div" :data="$divisi" selected="{{ $karyawan->kode_div }}" key="kode_div" textShow="nama_div"
        upperCase="true" />
    </div>
    ...
    <div class="form-group mb-3">
        ...
        <select name="status_karyawan" id="status_karyawan" class="form-select">
            ...
            <option value="K" {{ $karyawan->status_karyawan == 'K' ? 'selected' : '' }}>PKWT</option>
            <option value="T" {{ $karyawan->status_karyawan == 'T' ? 'selected' : '' }}>PKWTT</option>
        </select>
    </div>
